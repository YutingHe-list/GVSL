...
def correspondence(atlas_img, atlas_lab, unlab_img, warp_lab, corr):
    smooth = 1e-6
    atlas_img_proto = torch.mean(torch.sum(atlas_img * atlas_lab, dim=(2, 3, 4)) / (torch.sum(atlas_lab, dim=(2, 3, 4)) + 1e-6), dim=0)
    unlab_index = torch.where(torch.sum(warp_lab[:, 1:, :, :, :], dim=1, keepdim=True) > 0)
    unlab_img_roi = unlab_img[:, :, unlab_index[2], unlab_index[3], unlab_index[4]]
